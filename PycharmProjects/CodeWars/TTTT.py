...
def reportTime(string_time: str, report_path: str = ""):  # convert string to time in the given report
    ...
    if (len(t) == 1):  # if there are no delimiters in the time
        ...
        if (len(t) == 5) or (len(t) == 6):
            return datetime.time(int(t[:-4]), int(t[-4:-2]),
...
def rep2(report):
    times = [str(reportTime(t, "")) for t in report[0]]
    report2 = [["Time"] + times, ["Stage"] + report[1], ["Comments"], ["Doctor"], ["Diagnosis"],
               ["Position"], ["Medicine"], ["Overall comment"]]
    return report2
...
if __name__ == "__main__":
    ...
    REPORTS = [[rep2(myPy.readCSV(report)), myPy.fileFromPath(report)] for report in REPORTS]
    REPORTS = [[r[0], r[1][3:]] if '(' in r[1] else [r[0], r[1]] for r in REPORTS]

    path_to_save = "Z:\\Tetervak\\Reports\\reports 2.0"
    for r in REPORTS:
        myPy.write2csv(r[0], r[1], path_to_save)
    print("Complete")
