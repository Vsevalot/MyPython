...
class EEG(object):
    ...
    def save2arb(self, path_to_folder):
        with open("%s/%s_%.3fVpp.arb" % (path_to_folder, self._name[:6], self.amplitude), 'w') as file:
...
class ConvertionApp(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()

    def getEEG(self):
        path_to_file = QFileDialog.getOpenFileName(self, "Choose a recXXX_YYYYMMDD_HHMMSS.csv file",
                                                   "Z:/Tetervak")[0]
        if path_to_file == '':  # if no file selected
            return
        else:
            if path_to_file[-3:] != "csv":
                print("Choose a csv file with eeg and impedance")
                return
            eeg2arb(path_to_file)
            print(".arb files generated")

    def initUI(self):
        self.setGeometry(500, 500, 280, 170)
        self.setWindowTitle("eeg2arb")
        self.correct_btn = QPushButton("Choose an eeg file", self)
        self.correct_btn.clicked.connect(self.getEEG)
        self.correct_btn.setGeometry(50, 50, 200, 50)
        self.show()


def eeg2arb(path_to_csv):
    path_to_folder = ""
    ...
    for width in diapasons:
        ...
        plt.savefig("%s/%s_%.3fVpp_%.1f %c.jpg" % (path_to_folder, eeg._name[:6], eeg.amplitude, eeg.quality, '%'))
        print("%s complete,quantization step = %f mkV, quality = %.1f %c" %
              (eeg._name, eeg.quantization_step, eeg.quality, '%'))


if __name__ == "__main__":
    path_to_csv = ""  # Path to a csv eeg file here
    if path_to_csv != "":
        eeg2arb(path_to_csv)
        exit(0)

    # If not path defined -> call GUI
