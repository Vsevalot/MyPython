...
def give_source(data_source=''):
    ...
    if data_source[-4:] == 'json':
        ...
        print('Give a path to a dataSource json file or set argument for give_source() to "accelerometer" or "power"'
              ' to use default lab data sources')
        exit(0)
...
if __name__ == "__main__":
    ...
    accelerometer_source = fit_users.dataSources().create(userId='me', body=give_source("")).execute()
    ...
    # Make a request to create a dataSource. Argument body expects dict of your dataSource
    power_source = fit_users.dataSources().create(userId='me', body=give_source("")).execute()
    ...
    for source in dataSources_list:
        print("dataSource, name : {}".format(source["name"]))

    # You can delete dataSources by dataStreamId
    sourceID = ''
    if sourceID != '':
        fit_users.dataSources().delete(userId='me', dataSourceId=sourceID).execute()

    print("Finished. You've successfully created accelerometer and eeg power data sources.")
    print("Read your json files, and if everything is clear, run the datasets.py script")